...
//
// Purpose: reads tris from the geometry file: proj1e_geometry.vtk.
//
std::vector<Triangle>
GetTriangles(void)
{
   ...
   virtual void RenderPiece(vtkRenderer *ren, vtkActor *act)
   {
      ...
      if(rendered != true){
        ...
        glBegin(GL_TRIANGLES);
          ...
          for (int j = 0; j < 3; j++){
            int offset = 3 * (int)(256 * tris[i].fieldValue[j]);
            glColor3ub(colors[offset], colors[offset + 1], colors[offset + 2]);
            glNormal3f(tris[i].normals[j][0], tris[i].normals[j][1], tris[i].normals[j][2]);
 ...
 public:
   ...
   virtual void RenderPiece(vtkRenderer *ren, vtkActor *act)
   {
     ...
     if (rendered != true){
        ...
        for (int i = 0; i < tris.size(); i++){
          for (int j = 0; j < 3; j++){
            glTexCoord1f(tris[i].fieldValue[j]);
            glNormal3f(tris[i].normals[j][0], tris[i].normals[j][1], tris[i].normals[j][2]);
            glVertex3f(tris[i].X[j], tris[i].Y[j], tris[i].Z[j]);
          }
        }
...
int main()
{
  ...
  // library. It may also do offset mapping, if scalars or other attributes
  // are defined.
  //
